...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (inc == null || undefined || "") return message.channel.send("메시지를 보낼 채널을 알려주세요.")
    if (!args[2]) return message.channel.send("메시지 내용을 입력해주세요.");
    const embed = new Discord.MessageEmbed()
      .setTitle("메시지를 일반 메시지로 보낼까요?")
      .setColor("#303135")
      .addField("메시지 내용", `\`\`\`\n${args.slice(2).join(" ")}\n\`\`\``)
